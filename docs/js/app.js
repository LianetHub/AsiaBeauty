...
$(function () {
	...
	function initTabs() {
		$(".tab-btn").on("click", (e) => {
			...
			$target.addClass("active").siblings().removeClass("active").closest(".tabs").find(".tab-content").removeClass("active").eq($target.index()).addClass("active");
		});
	}
	...
	function toggleLocking(lockClass) {
		...
		let lockPaddingValue = $body.hasClass(className) ? "0px" : window.innerWidth - $(".wrapper").width() + "px";
	...
	class HeaderMarqueeSwiper {
		...
		isOverflowing() {
			...
			return slidesWidth > this.container.clientWidth;
		}

		initializeSwiper() {
			if (!this.isInitialized) {
				this.swiperInstance = new Swiper(this.container, this.options);
	...
	if ($(".header__marquee-slider").length > 0) {
		new HeaderMarqueeSwiper(".header__marquee-slider", {
			slidesPerView: "auto",
			spaceBetween: 30,
			speed: 800,
			loop: false,
	...
	class ConditionsSwiper {
		constructor(selector, options = {}, { mode = "min", breakpoint = 767.98 } = {}) {
	...
	if ($(".catalog__carousel").length > 0) {
		$(".catalog__carousel").each(function (index, sliderEl) {
			new Swiper(sliderEl, {
				spaceBetween: 20,
				slidesPerView: "auto",
				speed: 800,
	...
	class CustomSelect {
        ...
        `);
			...
			this.$options.each((index, option) => {
				...
				const listItem = $("<li>").attr("role", "option").data("value", value).attr("aria-checked", isSelected).addClass("dropdown__list-item").text(text);
